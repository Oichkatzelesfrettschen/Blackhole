...
float hawkingTemperatureDirect(float mass) {
    ...
    return HBAR * c3 / (8.0 * PI * G_GRAV * mass * K_BOLTZ);
}
 ...
 */
vec3 hawkingSpectrumLUT(float temperature, sampler2D hawkingSpectrumLUT) {
    if (temperature <= 0.0) {
        return vec3(0.0);
    }
    ...
    vec3 rgb = texture(hawkingSpectrumLUT, vec2(u, 0.5)).rgb;
