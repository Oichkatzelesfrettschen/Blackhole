...
TEST(KerrOrbitConvention, SignedSpinTable) {
  ...
  for (const Row &row : kTable) {
    ...
    EXPECT_NEAR(blackhole::physics::NovikovThorneDisk::iscoRadius(row.aStar), row.iscoProgradeOverM, 1.0e-12);
    ...
    EXPECT_NEAR(physics::kerrIscoRadius(mass, a, false), physics::kerrIscoRadius(mass, -a, true),
                1.0e-9 * mGeom);
    EXPECT_NEAR(physics::kerrPhotonOrbitRetrograde(mass, a), physics::kerrPhotonOrbitPrograde(mass, -a),
                1.0e-9 * mGeom);
