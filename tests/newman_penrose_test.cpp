...
bool testSchwarzschild() {
  ...
  for (const double r : radii) {
    ...
    const std::string buf =
        std::format("r={:g} M: Re(Psi_2)={:.6e} expected={:.6e} relErr={:.2e} Im={:.2e}", r / mGeo,
                    psi2.re, expectedRe, relRe, relIm);
  ...
  if (ok) {
    check(true, "Re(Psi_2) = -M/r^3, Im(Psi_2) = 0 for a=0");
  }
  return ok;
}
...
bool testEquatorialImaginary() {
  ...
  const std::string buf = std::format("Im(Psi_2) = {:.3e} (expected 0)", psi2.im);
...
bool testPsi2ModSq() {
  ...
  for (const double *tc : testCases) {
    ...
    const std::string buf =
        std::format("r={:g} M, c={:.2f}: |Psi2|^2={:.6e} expected={:.6e} relErr={:.2e}", r / mGeo,
                    cosTheta, modsq, expect, relErr);
    if (relErr > 1.0e-12) {
      std::cout << "  [FAIL] " << buf << "\n";
      gAllPass = false;
      ok = false;
  ...
  if (ok) {
    check(true, "|Psi_2|^2 = M^2/Sigma^3 at all test points (relErr < 1e-12)");
  }
  return ok;
}
...
bool testKretschmannSchwarzschild() {
  ...
  for (const double r : radii) {
    ...
    const std::string buf = std::format("r={:g} M: K={:.6e} expected={:.6e} relErr={:.2e}",
  ...
  if (ok) {
    check(true, "K = 48 M^2/r^6 for a=0 (relErr < 1e-12)");
  }
  return ok;
}
...
bool testKretschmannEquatorial() {
  ...
  const std::string buf =
      std::format("K(a=0)={:.6e}, K(a*=0.68)={:.6e}, K(a*=0.99)={:.6e}, expected={:.6e}", k0, k068,
                  k099, expected);

  const double tol = 1.0e-12;
  check(std::abs(k0 - expected) / expected < tol, "K equatorial a=0 matches 48M^2/r^6", buf);
  check(std::abs(k068 - expected) / expected < tol, "K equatorial a*=0.68 matches 48M^2/r^6", buf);
  check(std::abs(k099 - expected) / expected < tol, "K equatorial a*=0.99 matches 48M^2/r^6", buf);
  return true;
}
...
bool testPetrovTypeDIdentity() {
  ...
  const double testCases[5][2] = {{3.5 * mGeo, 0.0},
                                  {5.0 * mGeo, 0.5},
                                  {10.0 * mGeo, -0.7},
                                  {2.0 * mGeo, 0.9},
                                  {50.0 * mGeo, 0.1}};
  ...
  for (const auto &tc : testCases) {
    ...
    const std::string buf =
        std::format("r={:g} M, c={:.1f}: relErr={:.2e}", tc[0] / mGeo, tc[1], relErr);
    if (relErr > 1.0e-12) {
      std::cout << "  [FAIL] " << buf << "\n";
      gAllPass = false;
      ok = false;
  ...
  if (ok) {
    check(true, "K = 48|Psi_2|^2 at all test points (relErr < 1e-12)");
  }
  return ok;
}
...
bool testKretschmannAtHorizon() {
  ...
  for (const double c : cosVals) {
    ...
    const std::string buf = std::format("K(r_+, cos={:g}) = {:.4e}", c, k);
  ...
  if (ok) {
    check(true, "K(r_+, theta) is finite and positive for all theta");
  }
  return ok;
}
...
bool testRingSingularityGuard() {
  ...
  check(psi2.re == 0.0 && psi2.im == 0.0, "kerrWeylPsi2 returns (0,0) at Sigma=0 (guard)");

  const double k = kerrKretschmann(0.0, 0.0, mGeo, a);
  check(!safeIsfinite(k) || k > 1.0e100, "kerrKretschmann returns +inf (or very large) at Sigma=0");
  return true;
}
...
bool testLargeRFalloff() {
  ...
  const std::string buf =
      std::format("K={:.6e}, 48M^2/r^6={:.6e}, relErr={:.2e}", k, expected, relErr);
...
bool testKinnersleyNVector() {
  ...
  std::string buf = std::format("n^t={:.4f}, n^r={:.4f}, n^phi={:.4f}", nT, nR, nPhi);
  ...
  buf = std::format("n^r={:.6e} exact={:.6e} relErr={:.2e}", nR, nRExact, relErr);
...
int main() try {
  ...
  testSchwarzschild();
  std::cout << "\n";
  testEquatorialImaginary();
  std::cout << "\n";
  testPsi2ModSq();
  std::cout << "\n";
  testKretschmannSchwarzschild();
  std::cout << "\n";
  testKretschmannEquatorial();
  std::cout << "\n";
  testPetrovTypeDIdentity();
  std::cout << "\n";
  testKretschmannAtHorizon();
  std::cout << "\n";
  testRingSingularityGuard();
  std::cout << "\n";
  testLargeRFalloff();
  std::cout << "\n";
  testKinnersleyNVector();
  std::cout << "\n";
