...
bool testLorentzToBetaRest() {
  ...
  const std::string buf = std::format("beta(gamma=1) = {:.2f}, expected 0", beta);
...
bool testLorentzToBetaExact() {
  ...
  const std::string buf = std::format("beta = {:.15f}, expected {:.15f}", beta, expected);
...
bool testLorentzToBetaUltraRel() {
  ...
  const std::string buf = std::format("beta(gamma=1e6) = {:.15f} (expected > 1 - 1e-12)", beta);
...
bool testDopplerTransverse() {
  ...
  const std::string buf = std::format("delta = {:.10f}, 1/gamma = {:.10f}", delta, expected);
...
bool testDopplerHeadOn() {
  ...
  const std::string buf = std::format("delta(head-on) = {:.4f}, gamma = {:.1f}", delta, gamma);
...
bool testDopplerReceding() {
  ...
  const std::string buf =
      std::format("delta(receding) = {:.5f}, 1/gamma = {:.3f}", delta, transverse);
...
bool testHorizonOmegaSchwarzschild() {
  ...
  const std::string buf = std::format("Omega_H(a=0) = {:.4e} rad/s (expected 0)", omegaH);
...
bool testHorizonOmegaMonotone() {
  ...
  const std::string buf =
      std::format("Omega_H(a*=0.5) = {:.4e}, Omega_H(a*=0.9) = {:.4e} rad/s", omegaLow, omegaHigh);
...
bool testBZPowerSchwarzschild() {
  ...
  const std::string buf = std::format("P_BZ(a=0) = {:.4e} erg/s (expected 0)", pBz);
...
bool testBZPowerSpinning() {
  ...
  const double pBz = blandfordZnajekPower(jet);
  const std::string buf = std::format("P_BZ(M87, a*=0.9) = {:.4e} erg/s (expected > 0)", pBz);
...
bool testLorentzFactorTerminal() {
  ...
  const std::string buf =
      std::format("gamma(r_far) = {:.8f}, gamma_inf = {:.1f}", gamma, jet.lorentzFactor);
...
bool testMadJetPowerSane() {
  ...
  const std::string buf = std::format("P_jet = {:.4e}, 0.01*Mdot*c^2 = {:.4e}, relErr = {:.2e}",
...
bool testMadJetPowerMadGtSane() {
  ...
  const std::string buf = std::format("P_MAD = {:.4e}, P_SANE = {:.4e} erg/s", pMad, pSane);
...
bool testMadLorentzFactorSane() {
  ...
  const std::string buf = std::format("Gamma(SANE) = {:.4f} (expected 2.0)", gamma);
...
bool testMadLorentzFactorMad() {
  ...
  const double gamma = madJetLorentzFactor(disk);
  const std::string buf = std::format("Gamma(MAD, a*=0.94) = {:.4f} (expected in (5, 20))", gamma);
  check(gamma > 5.0 && gamma < 20.0, "madJetLorentzFactor(Sgr A*, MAD) in (5, 20)", buf);
  return true;
}
...
bool testEruptionMadMidCycle() {
  ...
  const std::string buf = std::format("phase = {:.4f} (expected in [0.4, 0.6])", phase);
...
bool testVariabilitySane() {
  ...
  const std::string buf = std::format("variability(SANE) = {:.4f} (expected 1.0)", v);
...
bool testMadTemperatureHeating() {
  ...
  const std::string buf = std::format("T_MAD = {:.4e} K, T_base = {:.4e} K", tMad, tBase);
...
int main() try {
  ...
  testLorentzToBetaRest();
  std::cout << "\n";
  testLorentzToBetaExact();
  std::cout << "\n";
  testLorentzToBetaUltraRel();
  std::cout << "\n";
  testDopplerTransverse();
  std::cout << "\n";
  testDopplerHeadOn();
  std::cout << "\n";
  testDopplerReceding();
  std::cout << "\n";
  testHorizonOmegaSchwarzschild();
  std::cout << "\n";
  testHorizonOmegaMonotone();
  std::cout << "\n";
  testBZPowerSchwarzschild();
  std::cout << "\n";
  testBZPowerSpinning();
  std::cout << "\n";
  testLorentzFactorTerminal();
  std::cout << "\n";
  testIsInsideJet();
  std::cout << "\n";
  testMadJetPowerSane();
  std::cout << "\n";
  testMadJetPowerMadGtSane();
  std::cout << "\n";
  testMadLorentzFactorSane();
  std::cout << "\n";
  testMadLorentzFactorMad();
  std::cout << "\n";
  testEruptionSane();
  std::cout << "\n";
  testEruptionMadMidCycle();
  std::cout << "\n";
  testVariabilitySane();
  std::cout << "\n";
  testMadTemperatureHeating();
  std::cout << "\n";
