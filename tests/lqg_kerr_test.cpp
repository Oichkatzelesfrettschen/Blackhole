...
void checkZeroScaleKerrMetric(double r, double theta, double m, double a, double l, double tol) {
  // g_tt
  EXPECT_NEAR(physics::lqgGtt(r, theta, m, a, l), kerrGttRef(r, theta, m, a), tol)
  ...
  const double sigma = sigmaRef;
  const double s = std::sin(theta);
  const double gtphRef = -2.0 * m * r * a * s * s / sigma;
  EXPECT_NEAR(physics::lqgGtph(r, theta, m, a, l), gtphRef, tol)
...
TEST(LqgKerr, ReducesToKerrAtLZero) {
  ...
  const double thetas[] = {std::numbers::pi / 6.0, std::numbers::pi / 3.0, std::numbers::pi / 2.0};
  ...
  for (double const a : spins) {
    for (double const r : radii) {
      ...
      // Delta
      EXPECT_NEAR(physics::lqgDelta(r, m, a, l), kerrDeltaRef(r, m, a), tol)
          << "Delta mismatch at l=0, a=" << a << " r=" << r;

      for (double const theta : thetas) {
        checkZeroScaleKerrMetric(r, theta, m, a, l, tol);
      }
    }
  }
}
...
TEST(LqgKerr, RegularAtOrigin) {
  ...
  for (double const l : ls) {
    // Effective mass at r = 0 must vanish
    EXPECT_NEAR(physics::lqgEffectiveMass(0.0, m, l), 0.0, tol) << "M_eff(r=0) != 0 at l=" << l;

    // LQG correction at r = 0 should be 1 (full correction)
    EXPECT_NEAR(physics::lqgCorrection(0.0, m, l), 1.0, tol) << "correction(r=0) != 1 at l=" << l;
...
TEST(LqgKerr, OuterHorizonBelowKerr) {
  ...
  for (double const l : ls) {
    const double rPlusLqg = physics::lqgOuterHorizon(m, a, l);
    EXPECT_TRUE(std::isfinite(rPlusLqg)) << "No horizon found for l=" << l;
    EXPECT_LT(rPlusLqg, rPlusKerr) << "LQG horizon >= Kerr horizon for l=" << l;
    EXPECT_GT(rPlusLqg, 0.0) << "LQG horizon <= 0 for l=" << l;
  }
}
...
TEST(LqgKerr, PhotonSphereShiftsInward) {
  ...
  const double rphKerr = physics::lqgPhotonSphereApprox(m, a, 0.0);
  EXPECT_GT(rphKerr, 0.0) << "Kerr photon sphere <= 0";

  // LQG photon sphere should be smaller.
  const double lEht = 0.3 * m;
  const double rphLqg = physics::lqgPhotonSphereApprox(m, a, lEht);
  EXPECT_LT(rphLqg, rphKerr) << "LQG photon sphere not below Kerr at l=0.3M";

  // Correction should be small: < 10% (EHT observational bound).
  const double frac = (rphKerr - rphLqg) / rphKerr;
