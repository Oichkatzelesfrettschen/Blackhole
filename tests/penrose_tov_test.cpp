...
bool testPenroseEfficiencySchwarzschild() {
  ...
  const std::string buf = std::format("eta(a*=0) = {:.8f} (expected 0)", eta);
...
bool testPenroseEfficiencyExtremal() {
  ...
  const std::string buf = std::format("eta(a*=1) = {:.10f}, expected {:.10f}", eta, expected);
...
bool testPenroseEfficiencyMonotone() {
  ...
  const std::string buf =
      std::format("eta(0,0.5,0.9,1) = {:.4f}, {:.4f}, {:.4f}, {:.4f}", eta0, eta5, eta9, eta1);
...
bool testIrreducibleMassSchwarzschild() {
  ...
  const std::string buf = std::format("M_irr(a*=0) = {:.6e}, M = {:.6e} g", mIrr, mass);
...
bool testIrreducibleMassExtremal() {
  ...
  const std::string buf = std::format("M_irr(a*=1) = {:.10e}, M/sqrt(2) = {:.10e}", mIrr, expected);
  check(std::abs(mIrr - expected) < 1.0e-10 * mass, "irreducibleMass(M, 1) = M/sqrt(2)", buf);
  return true;
}
...
bool testRotationalEnergySchwarzschild() {
  ...
  const std::string buf = std::format("E_rot(a*=0) = {:.4e} erg (expected 0)", eRot);
...
bool testRotationalEnergyExtremal() {
  ...
  const std::string buf =
      std::format("E_rot(a*=1) = {:.6e}, M*c^2*(1-1/sqrt2) = {:.6e} erg", eRot, expected);
...
bool testHorizonAreaSchwarzschild() {
  ...
  const std::string buf = std::format(
      "A(a=0) = {:.6e} cm^2, 16*pi*M_geo^2 = {:.6e}, relErr = {:.2e}", area, expected, relErr);
...
bool testHorizonAreaDecreasesWithSpin() {
  ...
  const std::string buf = std::format("A(a*=0) = {:.4e}, A(a*=0.99) = {:.4e} cm^2", aLow, aHigh);
...
bool testSuperradiantLowFreq() {
  ...
  const std::string buf = std::format("omega={:.4e}, m*Omega_H={:.4e} rad/s", omegaLow, omegaH);
  check(isSuperradiant(omegaLow, 1, mass, aPhys), "isSuperradiant(omega << m*Omega_H) = true", buf);
  return true;
}
...
bool testSuperradiantHighFreq() {
  ...
  const std::string buf = std::format("omega={:.4e}, m*Omega_H={:.4e} rad/s", omegaHigh, omegaH);
  check(!isSuperradiant(omegaHigh, 1, mass, aPhys), "isSuperradiant(omega >> m*Omega_H) = false",
        buf);
  return true;
}
...
bool testSuperradiantNonspinning() {
  ...
  check(!isSuperradiant(omega, 1, mass, 0.0), "isSuperradiant = false for non-spinning BH (a=0)");
  return true;
}
...
bool testTovDmdrExact() {
  ...
  const std::string buf =
      std::format("dm/dr(r=1, rho=1) = {:.10f}, 4*pi = {:.10f}", result, expected);
...
bool testTovMassRange() {
  ...
  const std::string buf = std::format("M_star = {:.4f} M_sun (expected in (0.1, 10))", mSol);
...
bool testTidalDeformabilityPositive() {
  ...
  const std::string buf = std::format("Lambda(C=0.2) = {:.4e} (expected > 0)", lambda);
...
int main() try {
  ...
  testPenroseEfficiencySchwarzschild();
  std::cout << "\n";
  testPenroseEfficiencyExtremal();
  std::cout << "\n";
  testPenroseEfficiencyMonotone();
  std::cout << "\n";
  testIrreducibleMassSchwarzschild();
  std::cout << "\n";
  testIrreducibleMassExtremal();
  std::cout << "\n";
  testIrreducibleMassUpperBound();
  std::cout << "\n";
  testRotationalEnergySchwarzschild();
  std::cout << "\n";
  testRotationalEnergyExtremal();
  std::cout << "\n";
  testHorizonAreaSchwarzschild();
  std::cout << "\n";
  testHorizonAreaDecreasesWithSpin();
  std::cout << "\n";
  testSuperradiantLowFreq();
  std::cout << "\n";
  testSuperradiantHighFreq();
  std::cout << "\n";
  testSuperradiantNonspinning();
  std::cout << "\n";
  testTovDmdrExact();
  std::cout << "\n";
  testTovIntegrationConverges();
  std::cout << "\n";
  testTovMassRange();
  std::cout << "\n";
  testTidalLoveNumberBound();
  std::cout << "\n";
  testTidalDeformabilityPositive();
  std::cout << "\n";
