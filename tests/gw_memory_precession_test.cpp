...
void check(bool condition, const char *test, std::string_view detail = {}) {
  if (condition) {
    std::cout << "  [PASS] " << test << "\n";
  } else {
    std::cout << "  [FAIL] " << test;
...
constexpr double K_M1_SOLAR = 36.3;  // Msun
constexpr double K_M2_SOLAR = 29.1;  // Msun
constexpr double K_DIST_MPC = 410.0; // Mpc

[[nodiscard]] double mTotalG() {
  return (K_M1_SOLAR + K_M2_SOLAR) * M_SUN;
}
[[nodiscard]] double eta150914() {
  const double m1 = K_M1_SOLAR * M_SUN;
  const double m2 = K_M2_SOLAR * M_SUN;
  const double mT = m1 + m2;
  return (m1 * m2) / (mT * mT);
}
[[nodiscard]] double distCm() {
  return K_DIST_MPC * 1.0e6 * PARSEC;
}
...
bool testMemoryPositive() {
  ...
  const double h = gwMemoryStrain(mTotalG(), eta150914(),
                                  /*fGw=*/35.0, distCm(), /*iota=*/0.0);
  check(h > 0.0, "memory strain is positive", std::format("h_mem = {:.3e} (expected > 0)", h));
  return true;
}
...
bool testMemoryMonotonicallyIncreases() {
  ...
  const double mT = mTotalG();
  const double eta = eta150914();
  const double d = distCm();
  ...
  for (std::size_t i = 1; i < 5; ++i) {
    ...
    check(h > prev, "memory increases with frequency",
          std::format("h(f={:g}) = {:.3e}, h(f={:g}) = {:.3e}", freqs[i - 1], prev, freqs[i], h));
    prev = h;
  }
  return true;
}
...
bool testMemoryAngularFactor() {
  ...
  check(hFaceOn > hEdgeOn, "face-on > edge-on",
        std::format("h_face_on={:.3e}, h_edge_on={:.3e}", hFaceOn, hEdgeOn));
...
bool testMemoryEtaScaling() {
  ...
  check(std::abs(measured - expected) < 1.0e-10, "linear eta scaling",
        std::format("h2/h1 = {:.6f}, expected {:.6f}", measured, expected));
  return true;
}
...
bool testMemoryMassScaling() {
  ...
  check(std::abs(measured - expected) < 1.0e-10, "M^{5/3} mass scaling",
        std::format("h2/h1 = {:.6f}, expected 2^{{5/3}} = {:.6f}", measured, expected));
  return true;
}
...
bool testMemoryDeltaConsistency() {
  ...
  const double diff = gwMemoryStrain(mT, eta, fHi, d, 0.0) - gwMemoryStrain(mT, eta, fLo, d, 0.0);

  check(std::abs(delta - diff) < 1.0e-30, "gwMemoryDelta matches h(fHi)-h(fLo)",
        std::format("delta={:.6e}, diff={:.6e}", delta, diff));
...
bool testMemoryOrderOfMagnitude() {
  ...
  const double h = gwMemoryStrain(mTotalG(), eta150914(), 35.0, distCm(), 0.0);

  check(h > 1.0e-23 && h < 1.0e-21, "memory in [1e-23, 1e-21]",
        std::format("h_mem(35Hz) = {:.3e} [expect 1e-23..1e-21]", h));
  return true;
}
...
bool testChiPSingleSpin() {
  ...
  check(std::abs(cp - chi1Perp) < 1.0e-12, "chiP = chi1Perp (no secondary in-plane spin)",
        std::format("chiP = {:.6f}, expected {:.6f}", cp, chi1Perp));
  return true;
}
...
bool testChiPEqualMass() {
  ...
  check(std::abs(cp - expected) < 1.0e-12, "chiP = max(chi1, chi2) at equal mass",
...
bool testChiPDownweighting() {
  ...
  const double expectedMax = std::max(cp1, cp2); // = 0.80
  check(cp < expectedMax, "chiP < max(chi1,chi2) for q < 1",
        std::format("chiP = {:.6f} < max = {:.6f}", cp, expectedMax));
...
bool testOpeningAngleAligned() {
  ...
  check(std::abs(beta) < 1.0e-12, "beta = 0 for aligned spins",
        std::format("beta = {:.6f} rad (expected 0)", beta));
  return true;
}
...
bool testOpeningAngleInPlane() {
  ...
  check(std::abs(beta - expected) < 1.0e-12, "beta = pi/2 for in-plane spin",
        std::format("beta = {:.6f}, expected {:.6f}", beta, expected));
  return true;
}
...
bool testPrecessionPhaseSign() {
  ...
  check(alpha > 0.0, "precession phase > 0 for chi_eff > 0",
        std::format("alpha = {:.4f} rad (expected > 0)", alpha));

  // Retrograde (chi_eff < 0) should give negative precession phase
  const double alphaRetro = simplePrecessionPhase(mT, eta, -chiEff, f);
  check(alphaRetro < 0.0, "precession phase < 0 for chi_eff < 0",
        std::format("alpha_retro = {:.4f} rad (expected < 0)", alphaRetro));
  return true;
}
...
bool testPrecessionPhaseDecreaseWithFrequency() {
  ...
  for (std::size_t i = 1; i < 5; ++i) {
    ...
    check(alpha < prev, "alpha decreases with frequency (1/f scaling)",
          std::format("alpha(f={:g}) = {:.4f} < alpha(f={:g}) = {:.4f} (1/f decrease)", freqs[i],
                      alpha, freqs[i - 1], prev));
    prev = alpha;
  }
  ...
  const double accumulated =
      simplePrecessionPhase(mT, eta, chiEff, fLo) - simplePrecessionPhase(mT, eta, chiEff, fHi);
  check(accumulated > 0.0, "accumulated precession from fLo to fHi is positive",
        std::format("accumulated precession from {:g} to {:g} Hz = {:.2f} rad", fLo, fHi,
                    accumulated));
  return true;
}
...
bool testPolarizationIsometry() {
  ...
  const double angles[] = {
      0.0, 0.3, 0.7, 1.2, std::numbers::pi / 4.0, std::numbers::pi / 2.0, std::numbers::pi};
  for (double const alpha : angles) {
    double hpPrec = 0.0;
    double hcPrec = 0.0;
    ...
    check(err < 1.0e-14, "polarization invariant preserved",
          std::format("alpha={:.3f} rad: rel err in h+^2+hx^2 = {:.2e} (expected < 1e-14)", alpha,
                      err));
  }
  return true;
}
...
int main() try {
  ...
  testMemoryPositive();
  std::cout << "\n";
  testMemoryMonotonicallyIncreases();
  std::cout << "\n";
  testMemoryAngularFactor();
  std::cout << "\n";
  testMemoryEtaScaling();
  std::cout << "\n";
  testMemoryMassScaling();
  std::cout << "\n";
  testMemoryDeltaConsistency();
  std::cout << "\n";
  testMemoryOrderOfMagnitude();
  std::cout << "\n";
  testMemoryDegenerateInputs();
  std::cout << "\n";
  ...
  testChiPEqualMass();
  std::cout << "\n";
  testChiPDownweighting();
  std::cout << "\n";
  testChiPZeroSpins();
  std::cout << "\n";
  testOpeningAngleAligned();
  std::cout << "\n";
  testOpeningAngleInPlane();
  std::cout << "\n";
  testPrecessionPhaseSign();
  std::cout << "\n";
  testPrecessionPhaseDecreaseWithFrequency();
  std::cout << "\n";
  testPolarizationIsometry();
  std::cout << "\n";
