 ...
 * @brief Validation tests for polarized Stokes I,Q,U,V transport.
 *
...
void testLinPolFrac() {
  ...
  const StokesVector s1 = {.i = 1.0, .q = 0.6, .u = 0.8, .v = 0.0}; // pol = 1.0, frac = 1.0
  check(nearRel(s1.linPolFrac(), 1.0, 1.0e-12),
        "linPolFrac: fully linearly polarized (Q,U)=(0.6,0.8)");
  const StokesVector s2 = {.i = 2.0, .q = 0.0, .u = 0.0, .v = 0.0}; // unpolarized
  check(near(s2.linPolFrac(), 0.0, 1.0e-15), "linPolFrac: zero for unpolarized I");
  const StokesVector s3 = {.i = 4.0, .q = 2.0, .u = 0.0, .v = 0.0}; // 50% linear
  check(nearRel(s3.linPolFrac(), 0.5, 1.0e-12), "linPolFrac: 50% for Q=I/2, U=0");
}

void testEvpa() {
  // Test 2: EVPA = 0.5*atan2(U,Q)
  const StokesVector sq = {.i = 1.0, .q = 1.0, .u = 0.0, .v = 0.0}; // EVPA = 0
  check(near(sq.evpa(), 0.0, 1.0e-14), "evpa: chi=0 for Q>0, U=0");
  const StokesVector su = {.i = 1.0, .q = 0.0, .u = 1.0, .v = 0.0}; // EVPA = pi/4
  check(nearRel(su.evpa(), std::numbers::pi / 4.0, 1.0e-12), "evpa: chi=pi/4 for Q=0, U>0");
  const StokesVector sq45 = {.i = 1.0, .q = -1.0, .u = 0.0, .v = 0.0}; // EVPA = pi/2 or -pi/2
...
void testRotateEvpa() {
  ...
  const StokesVector s = {.i = 1.0, .q = 1.0, .u = 0.0, .v = 0.3};
...
void testTotalPolFrac() {
  // Test 4: totalPolFrac = sqrt(Q^2+U^2+V^2)/I
  const StokesVector s = {.i = 2.0, .q = 1.0, .u = 1.0, .v = 1.0};
...
void testStokesStepIEqualsRteStep() {
  ...
  const StokesEmission em = {.jI = jI, .jQ = 0.0, .jU = 0.0, .jV = 0.0};
  const StokesVector sIn = {.i = i0, .q = 0.0, .u = 0.0, .v = 0.0};
  const StokesVector sOut = stokesStep(sIn, em, alpha, 0.0, ds);

  // Compare with rteStep
  const RteState rte = rteStep({.iNu = i0, .tau = 0.0, .sCm = 0.0}, jI, alpha, ds);
...
void testPureFaradayRotationPreservesI() {
  // Test 6: pure Faraday rotation (alpha_I=0): I,V unchanged; Q,U rotate
  const StokesVector s0 = {.i = 1.0, .q = 1.0, .u = 0.0, .v = 0.1};
...
void testFaradayRotationConservesLinPol() {
  // Test 8: linear polarization fraction conserved under pure Faraday rotation
  const StokesVector s0 = {.i = 2.0, .q = 1.0, .u = 0.5, .v = 0.0};
...
void testPureAbsorptionDecay() {
  // Test 9: pure absorption (rho_V=0, j=0): all Stokes decay as exp(-alpha*L)
  const StokesVector s0 = {.i = 1.0, .q = 0.5, .u = 0.3, .v = 0.2};
...
void testThickSlabEquilibrium() {
  ...
  const StokesEmission em = {.jI = jI, .jQ = 0.0, .jU = 0.0, .jV = 0.0};
  const StokesVector s1 = stokesStep({.i = 0.0, .q = 0.0, .u = 0.0, .v = 0.0}, em, alpha, 0.0, ds);
  check(nearRel(s1.i, jI / alpha, 1.0e-6), "stokesStep: thick slab I -> j_I/alpha_I");
}

void testZeroStepPreservesState() {
  // Test 11: ds=0 leaves state unchanged
  const StokesVector s0 = {.i = 1.23e-7, .q = 0.42e-7, .u = -0.1e-7, .v = 0.05e-7};
  const StokesEmission em = {.jI = 1.0e-9, .jQ = 0.5e-9, .jU = 0.0, .jV = 0.0};
...
void testExactMatchesRK4ForSimplifiedK() {
  ...
  const FaradayPropagation kProp = {
...
void testPathIntegrationMatchesSequential() {
  // Test 14: integrateStokesPath reproduces sequential stokesStep
  const StokesEmission em = {.jI = 1.5e-9, .jQ = 0.6e-9, .jU = 0.3e-9, .jV = 0.0};
  const FaradayPropagation fp = {.alphaI = 2.0e-14,
                                 .alphaQ = 0.0,
                                 .alphaV = 0.0,
                                 .rhoV = 5.0e-10,
                                 .rhoQ = 0.0,
  ...
  std::vector<StokesEmission> const ems(stepCount, em);
  std::vector<FaradayPropagation> const fps(stepCount, fp);
...
void testAccumulatedFaradayRotation() {
  ...
  const double totalPhi = rhoV * ds * stepCount; // expected total Faraday rotation
...
void testPolarizationBoundSatisfied() {
  // Test 16: stokesPolarizationBound: Q^2+U^2+V^2 <= I^2 after optically evolved state
  const StokesEmission em = {
      .jI = 1.0e-9, .jQ = 0.6e-9, .jU = 0.0, .jV = 0.0}; // Pi_Q = 0.6 (unphysical, tests bound)
  ...
  check(stokesPolarizationBound(s, 1.0e-8),
        "polarizationBound: satisfied at thick-slab equilibrium");
}

void testPolarizationBoundThickSlab() {
  // Test 17: bound holds when Pi < 1 (physical emission)
  const StokesEmission em = {.jI = 1.0e-9, .jQ = 0.7e-9, .jU = 0.0, .jV = 0.0}; // Pi = 0.7 < 1
  const double alpha = 1.0e-10;
...
void testLinearPolarFracPowerLaw() {
  // Test 18: (p+1)/(p+7/3); p=2 -> 3/(2+7/3) = 3/(13/3) = 9/13 ~ 0.692; p=3 -> 4/(16/3) = 12/16 =
  // 0.75
...
void testPolarizedEmissionVector() {
...
void testPolarizedEmissionEvpa() {
...
void testFaradayRotationCoeffScaling() {
...
void testFaradayRotationCoeffDegenerate() {
...
void testRelativisticFaradaySuppression() {
...
void testGRTransformStokes() {
  // Test 26: grTransformStokes: all four components scale as g^3
  const StokesVector s0 = {.i = 1.0e-6, .q = 0.5e-6, .u = 0.3e-6, .v = 0.1e-6};
  const double g = 0.8;
...
void testFaradayRotateStokes() {
  // Test 27: faradayRotateStokes rotates EVPA; I,V unchanged
  const StokesVector s0 = {.i = 1.0, .q = 1.0, .u = 0.0, .v = 0.2};
...
void testGRParallelTransportRotate() {
  // Test 28: grParallelTransportRotate preserves I and V; rotates EVPA
  const StokesVector s0 = {.i = 1.0, .q = 0.5, .u = 0.5, .v = 0.1};
