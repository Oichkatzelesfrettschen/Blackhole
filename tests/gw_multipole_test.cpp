...
void check(bool condition, std::string_view test, std::string_view detail = {}) {
  if (condition) {
    std::cout << "  [PASS] " << test << "\n";
  } else {
...
bool testGenericMatchesDominant() {
  ...
  for (const double a : spins) {
    ...
    const std::string buf =
        std::format("a*={:.2f}: scalar={:.4g} Hz generic={:.4g} Hz relErr={:.2e}", a, fScalar,
                    fGeneric, relErr);
...
bool testL33SchwarzchildLimit() {
  ...
  std::cout << "  omega_R*M = " << omRm << " (exact: " << exact << ", err: " << relErr * 100
            << "%)\n";

  /* Fit is accurate to ~1.4%; allow 2% tolerance */
  check(relErr < 0.02, "(3,3,0) Schw. limit within 2%", "omega_R*M must match Leaver 1985 to 2%");
...
bool testL33MonotonicFrequency() {
  ...
  for (const double a : spins) {
    ...
    if (f <= prevF) {
      monotone = false;
      std::cout << "  NOT monotone at a*=" << a << " prev=" << prevF << " curr=" << f << "\n";
    }
    prevF = f;
  }
...
bool testInspiralAngularPatterns() {
  ...
  check(std::abs(f22FaceOn - 1.0) < 1.0e-12, "F+(2,2) = 1 face-on");
  check(f33FaceOn == 0.0, "F+(3,3) = 0 face-on (sin iota = 0)");
  check(f44FaceOn == 0.0, "F+(4,4) = 0 face-on (sin^2 iota = 0)");
  ...
  check(std::abs(f22EdgeOn - 0.5) < 1.0e-12, "F+(2,2) = 0.5 edge-on");
  check(std::abs(f33EdgeOn - 0.5) < 1.0e-12, "F+(3,3) = 0.5 edge-on");
  check(std::abs(f44EdgeOn - 0.5) < 1.0e-12, "F+(4,4) = 0.5 edge-on");
  ...
  check(std::abs(fx22FaceOn - 1.0) < 1.0e-12, "Fx(2,2) = 1 face-on");
  check(fx33FaceOn == 0.0, "Fx(3,3) = 0 face-on");
...
bool testInspiralModeFractionsEqualMass() {
  ...
  const std::string buf = std::format("(4,4) fraction = {:.4e}", r44);
...
bool testInspiralModeFractionsUnequalMass() {
  ...
  const std::string buf = std::format("v={:.3f} delta={:.3f}: r33={:.4f} r21={:.4f} r44={:.4f}", v,
                                      delta, r33, r21, r44);
  std::cout << "  " << buf << "\n";
...
bool testInspiralMultimodeEqualMass() {
  ...
  double hPlus = 0.0;
  double hCross = 0.0;
  gwInspiralStrainMultimode(mc, mTotal, etaVal, delta, fGw, d, iota, phiOrb, hPlus, hCross);
  ...
  const double hPlusExpected =
      h22amp * ((gwInspiralAngularPlus(2, 2, iota) * std::cos(2.0 * phiOrb)) +
                (r44 * gwInspiralAngularPlus(4, 4, iota) * std::cos(4.0 * phiOrb)));
  const double hCrossExpected =
      h22amp * ((gwInspiralAngularCross(2, 2, iota) * std::sin(2.0 * phiOrb)) +
                (r44 * gwInspiralAngularCross(4, 4, iota) * std::sin(4.0 * phiOrb)));

  const std::string buf =
      std::format("hPlus={:.4e} expected={:.4e} relErr={:.2e}", hPlus, hPlusExpected,
                  std::abs(hPlus - hPlusExpected) / std::abs(hPlusExpected));
...
bool testInspiralMultimodeUnequalMass() {
  ...
  const double h22amp = gwStrain1pn(mc, etaVal, fGw, d);
  const double hPlus22 = h22amp * gwInspiralAngularPlus(2, 2, iota) * std::cos(2.0 * phiOrb);
  const double hCross22 = h22amp * gwInspiralAngularCross(2, 2, iota) * std::sin(2.0 * phiOrb);
  ...
  const std::string buf = std::format("power_multi={:.4e} power_22={:.4e} ratio={:.4f}", powerMulti,
                                      power22, powerMulti / power22);
  std::cout << "  " << buf << "\n";
...
int main() try {
  ...
  testFitParamsLookup();
  std::cout << "\n";
  testGenericMatchesDominant();
  std::cout << "\n";
  testL33SchwarzchildLimit();
  std::cout << "\n";
  testL33MonotonicFrequency();
  std::cout << "\n";
  testL44SchwarzchildLimit();
  std::cout << "\n";
  testModeAmplitudeHierarchy();
  std::cout << "\n";
  testMultimodeRingdown();
  std::cout << "\n";
  testEqualMassSymmetry();
  std::cout << "\n";
  testInspiralAngularPatterns();
  std::cout << "\n";
  testInspiralModeFractionsEqualMass();
  std::cout << "\n";
  testInspiralModeFractionsUnequalMass();
  std::cout << "\n";
  testInspiralMultimodeEqualMass();
  std::cout << "\n";
  testInspiralMultimodeUnequalMass();
  std::cout << "\n";
