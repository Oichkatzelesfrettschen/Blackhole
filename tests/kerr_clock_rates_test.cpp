...
constexpr double kHalfPi = 0.5 * physics::PI;
...
TEST(KerrClockRates, EquatorialTableAtSixM) {
  ...
  EXPECT_NEAR(physics::kerrZamoLapse(r, kHalfPi, h.mass, a), 0.81798157138940855, 1.0e-12);
  const std::optional<double> staticRate = physics::kerrStaticTimeDilation(r, kHalfPi, h.mass, a);
  ASSERT_TRUE(staticRate.has_value());
  EXPECT_NEAR(*staticRate, 0.81649658092772603, 1.0e-12);
  ...
  EXPECT_NEAR(*pro, 0.74344405871481959, 1.0e-12);
  EXPECT_NEAR(*retro, 0.65451153007973629, 1.0e-12);
}
...
TEST(KerrClockRates, ErgoregionHasNoStaticObserver) {
  ...
  for (const Row &row : rows) {
    ...
    EXPECT_NEAR(physics::kerrZamoLapse(r, kHalfPi, h.mass, a), row.zamo, 1.0e-12) << row.rOverM;
    const std::optional<double> pro = physics::kerrCircularOrbitTimeDilation(r, h.mass, a, true);
    ASSERT_TRUE(pro.has_value()) << row.rOverM;
    EXPECT_NEAR(*pro, row.prograde, 1.0e-12) << row.rOverM;
  ...
  EXPECT_FALSE(physics::kerrStaticTimeDilation(1.6 * h.mGeom, kHalfPi, h.mass, a).has_value());
  EXPECT_FALSE(physics::kerrStaticTimeDilation(1.8 * h.mGeom, kHalfPi, h.mass, a).has_value());
  EXPECT_NEAR(*physics::kerrStaticTimeDilation(3.0 * h.mGeom, kHalfPi, h.mass, a),
...
TEST(KerrClockRates, SchwarzschildLimit) {
  ...
  for (double const x : {3.5, 6.0, 20.0}) {
    ...
    EXPECT_NEAR(*physics::kerrStaticTimeDilation(r, 0.7, h.mass, 0.0), staticRate, 1.0e-14);
    const double orbit = std::sqrt(1.0 - (3.0 / x));
    EXPECT_NEAR(*physics::kerrCircularOrbitTimeDilation(r, h.mass, 0.0, true), orbit, 1.0e-14);
    EXPECT_NEAR(*physics::kerrCircularOrbitTimeDilation(r, h.mass, 0.0, false), orbit, 1.0e-14);
  }
  EXPECT_NEAR(*physics::kerrCircularOrbitTimeDilation(6.0 * h.mGeom, h.mass, 0.0, true),
...
TEST(KerrClockRates, SignedSpinReflection) {
  ...
  const double r = 6.0 * h.mGeom;
  const double a = 0.9 * h.mGeom;
  EXPECT_DOUBLE_EQ(*physics::kerrCircularOrbitTimeDilation(r, h.mass, -a, true),
                   *physics::kerrCircularOrbitTimeDilation(r, h.mass, a, false));
}
...
TEST(KerrClockRates, RedshiftThroughErgoregion) {
  ...
  const double z = physics::kerrRedshift(r, kHalfPi, h.mass, a);
  EXPECT_NEAR(z, (1.0 / 0.19695340720394075) - 1.0, 1.0e-10);
  EXPECT_FALSE(physics::kerrStaticRedshift(r, kHalfPi, h.mass, a).has_value());
  const double rPlus = physics::kerrOuterHorizon(h.mass, a);
  EXPECT_TRUE(std::isinf(physics::kerrRedshift(rPlus, kHalfPi, h.mass, a)));
  EXPECT_EQ(physics::kerrZamoLapse(rPlus, kHalfPi, h.mass, a), 0.0);

  // a = 0: identical to the Schwarzschild static redshift.
  const double r10 = 10.0 * h.mGeom;
  EXPECT_NEAR(physics::kerrRedshift(r10, kHalfPi, h.mass, 0.0),
              (1.0 / std::sqrt(1.0 - 0.2)) - 1.0, 1.0e-14);

  std::vector<float> zBatch;
  physics::kerrRedshiftBatch({rPlus, r, 2.0 * rPlus}, kHalfPi, h.mass, a, zBatch);
