...
bool testGFactorSchwarzschild() {
  ...
  const std::string buf = std::format("g={:.8f}, expected={:.8f}", g, expected);
...
bool testGFactorApproachingBlueshifted() {
  ...
  const std::string buf =
      std::format("g(r=10, phi=pi/2, a=0, iota=pi/2) = {:.6f} (expected > 1)", g);
...
bool testGFactorRecedingRedshifted() {
  ...
  const std::string buf =
      std::format("g(r=10, phi=3pi/2, a=0, iota=pi/2) = {:.6f} (expected < 1)", g);
...
bool testGFactorFlatSpaceLimit() {
  ...
  const std::string buf = std::format("g(r=1e6, phi=0, a=0, iota=pi/4) = {:.8f} (expected ~1)", g);
...
bool testProfileNormalization() {
  ...
  const std::string buf = std::format("integral = {:.4f} (expected 1.0 +/- 0.01)", integral);
...
bool testProfilePositive() {
  ...
  const bool allPositive = std::ranges::all_of(
      profile, [](const auto &point) { return point.first > 0.0 && point.second >= 0.0; });
...
bool testProfileSpinRedshift() {
  ...
  const std::string buf =
      std::format("E_min(a*=0.0)={:.3f} keV, E_min(a*=0.9)={:.3f} keV", eMinA0, eMinA9);
...
bool testProfileInclinationBroadening() {
  ...
  const std::string buf =
      std::format("width_face={:.3f} keV, width_edge={:.3f} keV", widthFace, widthEdge);
...
bool testGMinSchwarzschild() {
  ...
  const std::string buf = std::format("gMin={:.8f}, expected {:.8f}", gm, expected);
...
bool testProfileFluxNonNegative() {
  ...
  const bool allOk =
      std::ranges::all_of(profile, [](const auto &point) { return point.second >= 0.0; });
...
int main() try {
  ...
  testGFactorSchwarzschild();
  std::cout << "\n";
  testGFactorApproachingBlueshifted();
  std::cout << "\n";
  testGFactorRecedingRedshifted();
  std::cout << "\n";
  testGFactorFlatSpaceLimit();
  std::cout << "\n";
  testProfileNormalization();
  std::cout << "\n";
  testProfilePositive();
  std::cout << "\n";
  testProfileSpinRedshift();
  std::cout << "\n";
  testProfileInclinationBroadening();
  std::cout << "\n";
