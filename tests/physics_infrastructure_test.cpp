...
bool testComputeConservedLzRadial() {
  ...
  const ConservedQuantities cq =
      computeConserved(50.0, std::numbers::pi / 2.0, 1.0, -0.5, 0.0, 0.0, 1.0, 0.0);

  const std::string buf = std::format("L_z = {:.4e} (expected 0)", cq.lz);
  check(std::abs(cq.lz) < 1.0e-14, "L_z = 0 for radial geodesic in Schwarzschild", buf);
  return true;
}
...
bool testMonitorZeroDrift() {
  ...
  const std::string buf = std::format("drift = {:.4e} (expected 0)", drift);
...
bool testMonitorMaxDrift() {
  ...
  const std::string buf = std::format("maxDrift = {:.6e} (expected 0.01)", drift);
  check(std::abs(drift - 0.01) < 1.0e-10, "maxDrift = 0.01 for 1% E perturbation", buf);
  return true;
}
...
bool testCrossedRadiusYesNo() {
  ...
  check(yes && !no, "crossedRadius(5->3, 4)=true and crossedRadius(5->6, 4)=false");
  return true;
}
...
bool testHasTurningPointYesNo() {
  ...
  check(yes && !no, "hasTurningPoint(+1,-1)=true and hasTurningPoint(+1,+0.5)=false");
  return true;
}
...
bool testCrossedEquator() {
  ...
  const std::string buf =
      std::format("[1->2]: {}, [0.1->0.5]: {}  (pi/2={:.4f})", yes ? "true" : "false",
                  no ? "true" : "false", std::numbers::pi / 2.0);
...
bool testPhotonSubringIndex() {
  ...
  const std::array turningPointCounts{0, 1, 2, 100};
  check(std::ranges::all_of(turningPointCounts,
                            [](int count) { return photonSubringIndex(count) == count; }),
...
bool testBisectRadialCrossing() {
  ...
  const std::string buf = std::format(
      "r_cross = {:.10f} (expected 4.0), lambda = {:.10f} (expected 0.5)", ev.r, ev.lambda);
...
bool testM87Preset() {
  ...
  const std::string buf = std::format("mass={:.2e} Msun, spin={:.4f}", m87.massMsun, m87.spin);
...
bool testSgraPreset() {
  ...
  const std::string buf = std::format("mass={:.2e} Msun, spin={:.4f}", sgra.massMsun, sgra.spin);
...
bool testM87ShadowDiameter() {
  ...
  const std::string buf = std::format("shadow = {:.2f} uas  (EHT: 42+-3 uas)", shadow);
  check(shadow > 38.0 && shadow < 46.0, "M87* shadow diameter in [38, 46] uas", buf);
  return true;
}
...
bool testSgraAngularScale() {
  ...
  const std::string buf = std::format("angular scale = {:.3f} uas/r_g", scale);
  check(scale > 4.0 && scale < 6.0, "Sgr A* angular scale in [4, 6] uas/r_g", buf);
  return true;
}
...
bool testMassRoundTrip() {
  ...
  const std::string buf = std::format("M_SUN -> {:.10e} g (expected {:.10e} g)", mBack, mCgs);
  check(std::abs(mBack - mCgs) / mCgs < 1.0e-14, "mass CGS -> geom -> CGS round-trip for 1 Msun",
        buf);
  return true;
}
...
bool testSchwarzschildRadius() {
  ...
  const std::string buf =
      std::format("rS(1 Msun) = {:.10e} cm,  R_SCHW_SUN = {:.10e} cm", computed, R_SCHW_SUN);
  check(std::abs(computed - R_SCHW_SUN) / R_SCHW_SUN < 1.0e-14, "rS(1 Msun) == R_SCHW_SUN", buf);
  return true;
}
...
bool testAngleRoundTrip() {
  ...
  const std::string buf =
      std::format("angle={:.10e} -> {:.4f} uas -> {:.10e} rad", angle, uas, back);
  check(std::abs(back - angle) / angle < 1.0e-14, "radToUas / uasToRad round-trip", buf);
  return true;
}
...
bool testMassMsunToGeomRelation() {
  ...
  const double twiceRg = 2.0 * massMsunToGeom(1.0);
  const std::string buf =
      std::format("2*r_g = {:.10e} cm,  R_SCHW_SUN = {:.10e} cm", twiceRg, R_SCHW_SUN);
  check(std::abs(twiceRg - R_SCHW_SUN) / R_SCHW_SUN < 1.0e-14,
...
int main() try {
  ...
  testComputeConservedEnergy();
  std::cout << "\n";
  testComputeConservedLzRadial();
  std::cout << "\n";
  testMonitorZeroDrift();
  std::cout << "\n";
  testMonitorViolated();
  std::cout << "\n";
  testMonitorMaxDrift();
  std::cout << "\n";
  testCrossedRadiusYesNo();
  std::cout << "\n";
  testHasTurningPointYesNo();
  std::cout << "\n";
  testCrossedEquator();
  std::cout << "\n";
  testPhotonSubringIndex();
  std::cout << "\n";
  testBisectRadialCrossing();
  std::cout << "\n";
  testM87Preset();
  std::cout << "\n";
  testSgraPreset();
  std::cout << "\n";
  testM87ShadowDiameter();
  std::cout << "\n";
  testSgraAngularScale();
  std::cout << "\n";
  testMassRoundTrip();
  std::cout << "\n";
  testSchwarzschildRadius();
  std::cout << "\n";
  testAngleRoundTrip();
  std::cout << "\n";
  testMassMsunToGeomRelation();
  std::cout << "\n";
