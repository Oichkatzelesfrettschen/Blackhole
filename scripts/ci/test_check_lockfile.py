...
class LockfileTests(unittest.TestCase):
    def compare(self, committed, resolved):
        with tempfile.TemporaryDirectory() as directory:
            ...
            return subprocess.run(
                [sys.executable, str(Path(__file__).with_name("check_lockfile.py")),
                 *map(str, paths)], check=False, capture_output=True,
            ).returncode

    def test_export_timestamp_preserves_identity(self):
        self.assertEqual(self.compare(
            {"requires": ["imgui/1.92.5-docking#abc%1.2"]},
            {"requires": ["imgui/1.92.5-docking#abc%9.9"]},
        ), 0)

    def test_version_revision_and_removal_fail(self):
        for changed in (["imgui/1.92.5-docking#def%1.2"],
                        ["imgui/1.92.6#abc%1.2"], []):
    ...
    def test_build_dependency_drift_fails(self):
        self.assertEqual(self.compare(
            {"build_requires": ["cmake/3.31.6#abc%1.2"]},
            {"build_requires": ["cmake/3.31.6#def%1.2"]},
        ), 1)

    def test_lock_metadata_drift_fails(self):
