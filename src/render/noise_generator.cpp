...
NoiseGenerator::NoiseGenerator(const NoiseConfig &config) : config_(config) {
...
NoiseVolume NoiseGenerator::generate3D(uint32_t width, uint32_t height, uint32_t depth,
  ...
  generator_->GenUniformGrid3D(volume.data.data(), static_cast<int>(xOffset),
                               static_cast<int>(yOffset), static_cast<int>(zOffset),
                               static_cast<int>(width), static_cast<int>(height),
                               static_cast<int>(depth), config_.frequency, config_.seed);
...
std::vector<float> NoiseGenerator::generate2D(uint32_t width, uint32_t height,
                                              [[maybe_unused]] float zSlice) {
  std::vector<float> data(static_cast<size_t>(width) * height);

  // Generate 2D slice at specified Z
  generator_->GenUniformGrid2D(data.data(),
                               0, // xStart
                               0, // yStart
                               static_cast<int>(width), static_cast<int>(height), config_.frequency,
                               config_.seed);
