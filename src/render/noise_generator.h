...
struct NoiseVolume {
  ...
  void set(uint32_t x, uint32_t y, uint32_t z, float value) { data.at(index(x, y, z)) = value; }

  [[nodiscard]] size_t sizeBytes() const { return data.size() * sizeof(float); }
};
...
public:
   ...
   */
  NoiseVolume generate3D(uint32_t width, uint32_t height, uint32_t depth, float xOffset = 0.0f,
                         float yOffset = 0.0f, float zOffset = 0.0f);
   ...
   */
  std::vector<float> generate2D(uint32_t width, uint32_t height, float zSlice = 0.0f);
