...
void captureCompareParity(RenderState &rs, const CompareParityInputs &in) {
  ...
  if (++rs.compare.compareFrameCounter >= rs.compare.compareFrameStride) {
    rs.compare.compareStats = sampleTextureDiff(
        rs.targets.texBlackhole, rs.targets.texBlackholeCompare, rs.targets.renderWidth,
  ...
  if (rs.compare.captureCompareSnapshot) {
    ...
    if (readTextureRGBA(rs.targets.texBlackhole, rs.targets.renderWidth, rs.targets.renderHeight,
                        primary) &&
        readTextureRGBA(rs.targets.texBlackholeCompare, rs.targets.renderWidth,
                        rs.targets.renderHeight, secondary)) {
      ...
      rs.compare.compareLastExceeded =
          rs.compare.compareFullStats.valid &&
      ...
      if (rs.compare.compareWriteDiff) {
        writeDiffPpm(compareSnapshotPath(rs.compare.compareSnapshotIndex, "diff"), primary,
                     secondary, rs.targets.renderWidth, rs.targets.renderHeight,
                     rs.compare.compareDiffScale);
      }
      if (rs.compare.compareWriteSummary) {
        appendCompareSummary(compareSummaryPath(), rs.compare.compareSnapshotIndex, primaryTag,
                             secondaryTag, rs.targets.renderWidth, rs.targets.renderHeight,
                             rs.compare.compareFullStats, rs.compare.compareDiffScale,
                             rs.compare.compareWriteOutputs, rs.compare.compareWriteDiff,
                             rs.compare.compareThreshold, rs.compare.compareLastExceeded,
                             in.timeSec, rs.physicsCore.kerrSpin, in.grbModulationEnabled,
                             in.grbTimeSeconds, rs.compare.compareLastOutliers,
                             rs.compare.compareLastOutlierLimit, outlierFrac);
        appendCompareUniforms(compareUniformsPath(), rs.compare.compareSnapshotIndex, presetLabel,
                              in.interop, in.compareBaselineActive,
                              rs.compare.compareOverridesEnabled, in.backgroundEnabledEffective,
                              in.noiseReady, in.grmhdEnabled, in.spectralEnabled,
                              in.grbModulationEnabled, in.enablePhotonSphereEffective);
      }
