...
struct CamKeyframe {
  float timeSec;       ///< Absolute time in seconds [0, 180].
...
public:
  /**
   * @brief Evaluate the camera path at absolute time timeSec.
   *
   ...
   *
   * @param timeSec  Time in seconds, clamped to [0, K_CINEMATIC_DURATION_S].
   * @return Interpolated CamKeyframe.
   */
  [[nodiscard]] static CamKeyframe evaluate(float timeSec) noexcept {
    static constexpr std::size_t keyframeCount = K_CINEMATIC_KEYFRAMES.size();

    timeSec = std::fmaxf(0.0f, std::fminf(timeSec, K_CINEMATIC_DURATION_S));

    // Find segment index i: kf[i].timeSec <= timeSec < kf[i+1].timeSec
    std::size_t i = 0;
    for (std::size_t k = 0; k + 1 < keyframeCount; ++k) {
      if (timeSec < K_CINEMATIC_KEYFRAMES[k + 1].timeSec) {
        i = k;
        break;
      }
      i = k;
    }
    if (timeSec >= K_CINEMATIC_KEYFRAMES[keyframeCount - 1].timeSec) {
      return K_CINEMATIC_KEYFRAMES[keyframeCount - 1];
    }
    ...
    auto const &k3 = K_CINEMATIC_KEYFRAMES[(i + 2 < keyframeCount) ? i + 2 : keyframeCount - 1];

    float const span = k2.timeSec - k1.timeSec;
    float const u = (span > 0.0f) ? (timeSec - k1.timeSec) / span : 0.0f;
 ...
 * NewFrame and Render).
 *
 * @param timeSec       Current cinematic time [0, 180].
 ...
 * @param iscoRadius      ISCO radius in scene units.
 * @param frameIndex  Current 0-based frame index.
 * @param totalFrames Total frames to record (for progress bar).
 */
inline void renderCinematicOverlay(float timeSec, const CamKeyframe &kf, float camDist, float rs,
                                   float iscoRadius, int frameIndex, int totalFrames) {
  ImDrawList *dl = ImGui::GetForegroundDrawList();
  ImVec2 const disp = ImGui::GetIO().DisplaySize;
  float const displayWidth = disp.x;
  float const displayHeight = disp.y;

  // ---- Derived physics -------------------------------------------------
  float const mass = rs * 0.5f;
  ...
  float const tHawking = (mass > 1e-9f) ? (1.0f / (8.0f * std::numbers::pi_v<float> * mass)) : 0.0f;
  ...
  // =========================================================================
  {
    ...
    buffer = std::format("  Spin        a* = {:.4f}", static_cast<double>(aStar));
    text(tx, ty, colValue, buffer.c_str());
    ty += lineH;

    buffer = std::format("  Mass         M = {:.2f}   (r_s = {:.2f})", static_cast<double>(mass),
                         static_cast<double>(rs));
    text(tx, ty, colValue, buffer.c_str());
    ty += lineH;

    buffer = std::format("  Horizon     r+ = {:.4f} M", static_cast<double>(rPlus / mass));
    text(tx, ty, colValue, buffer.c_str());
    ty += lineH;

    buffer = std::format("  ISCO    r_ISCO = {:.4f} M", static_cast<double>(iscoRadius / mass));
    text(tx, ty, colValue, buffer.c_str());
    ty += lineH;

    buffer = std::format("  Camera        r = {:.3f} M", static_cast<double>(rCam / mass));
    ImU32 const rCol = (rCam < iscoRadius * 1.05f) ? colWarn : colValue;
    text(tx, ty, rCol, buffer.c_str());
    ty += lineH;

    buffer = std::format("  Redshift      z = {:.5f}", static_cast<double>(zCam));
    text(tx, ty, colValue, buffer.c_str());
    ty += lineH;

    buffer = std::format("  Time dilation   = {:.5f} x", static_cast<double>(dilCam));
    text(tx, ty, colValue, buffer.c_str());
    ty += lineH;

    buffer = std::format("  Omega_ff        = {:.5f} M^-1", static_cast<double>(omegaFF));
    text(tx, ty, colValue, buffer.c_str());
    ty += lineH;

    buffer = std::format("  T_Hawking       = {:.3e} M^-1", static_cast<double>(tHawking));
    text(tx, ty, colValue, buffer.c_str());
  }
  ...
  // =========================================================================
  {
    ...
    text(tx, ty, colEq, "  Sigma = r^2 + a^2 cos^2(theta)");
    ty += lineH;
    text(tx, ty, colEq, "  Delta = r^2 - r_s*r + a^2");
    ty += lineH;
    ty += 3.0f;
    text(tx, ty, colEq, "  E  = -(g_tt k^t + g_tphi k^phi)");
    ty += lineH;
    text(tx, ty, colEq, "  Lz =   g_tphi k^t + g_pp k^phi");
    ty += lineH;
    text(tx, ty, colEq, "  Q  = Sigma^2 (k^theta)^2");
    ty += lineH;
    text(tx, ty, colEq, "          - a^2 cos^2 + Lz^2/sin^2");
    ty += lineH;
    ty += 3.0f;
    text(tx, ty, colEq, "  R(r)  = [E(r^2+a^2) - aLz]^2");
    ty += lineH;
    text(tx, ty, colEq, "            - Delta[Q + (Lz - aE)^2]");
    ty += lineH;
    text(tx, ty, colEq, "  T(th) = Q + a^2 E^2 cos^2");
    ty += lineH;
  ...
  // =========================================================================
  {
    ...
    for (std::size_t row = 0; row < rows.size(); ++row) {
      ...
      if (td > 999.0f) {
        buffer = std::format(" -> inside horizon");
      } else {
        buffer = std::format(" -> {:.5f} x", static_cast<double>(td));
      }
      text(tx, ty, colValue, rows[row].label);
      text(tx + 148.0f, ty, colEq, buffer.c_str());
      ty += lineH;
    }
  }
  ...
  // =========================================================================
  {
    ...
    // Live ticker
    int const tMin = static_cast<int>(timeSec) / 60;
    int const tSec = static_cast<int>(timeSec) % 60;
    buffer = std::format("  T {}:{:02d}  |  Frame {} / {}  |  "
                         "Dist-to-horizon: {:.3f} M  |  "
                         "Redshift z: {:.4f}  |  "
                         "Time dilation: {:.4f} x  |  "
                         "T_Hawking: {:.3e} M^-1",
                         tMin, tSec, frameIndex, totalFrames,
                         static_cast<double>((rCam - rPlus) / mass), static_cast<double>(zCam),
                         static_cast<double>(dilCam), static_cast<double>(tHawking));
    text(tx, ty, colValue, buffer.c_str());
    ty += lineH + 3.0f;
