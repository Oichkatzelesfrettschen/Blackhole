...
// can be placed on the callback in order to get a stacktrace for the GL error. (enable together
// with GL_DEBUG_OUTPUT !)
...
void APIENTRY GLDebugMessageCallback(GLenum source, GLenum type, GLuint id, GLenum severity,
  ...
  if (std::strcmp(_severity, "NOTIFICATION") != 0) {
    printf("OpenGL error [%u]: %s of %s severity, raised from %s: %s\n", id, _type, _severity,
           _source, msg);
#ifdef _DEBUG
    __debugbreak();
#endif
