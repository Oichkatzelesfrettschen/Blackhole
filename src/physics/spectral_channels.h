 ...
 */
[[nodiscard]] inline double gaussianFilter(double frequency, double centerFrequency, double bandwidth) {
    if (bandwidth < 1e6) {
        return 0.0;
    }
...
[[nodiscard]] inline MultibandsFlux sampleSpectrumBands(const double* frequencies,
    ...
    for (int i = 0; i < nSamples - 1; ++i) {
        ...
        // Radio band (230 GHz)
        const double radioResponse = gaussianFilter(freqMid, RADIO_BAND.centerHz,
                                                    RADIO_BAND.bandwidthHz);
        fRadio += radioResponse * fluxMid * dFreq;

        // Optical band (540 nm ~ 5.6e14 Hz)
        const double opticalResponse = gaussianFilter(freqMid, OPTICAL_BAND.centerHz,
                                                      (OPTICAL_BAND.bandwidthNm / 1e9) *
                                                          OPTICAL_BAND.centerHz);
        fOptical += opticalResponse * fluxMid * dFreq;

        // X-ray band (0.5-10 keV)
        const double xrayCenter = XRAY_BAND.centerHz;
        const double xrayBandwidth = (energyToFrequency(XRAY_BAND.energyKevMax) -
                                      energyToFrequency(XRAY_BAND.energyKevMin));
