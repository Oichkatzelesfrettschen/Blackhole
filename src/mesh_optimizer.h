 ...
 * @param vertexCount Number of unique vertices
 * @param stride Bytes between consecutive vertex positions (usually sizeof(float)*3 or
 * sizeof(Vertex))
 * @param threshold Overdraw threshold (1.0 = no overdraw allowed, higher = more relaxed)
 * @return true if optimization was performed
 */
inline bool optimizeOverdraw(std::vector<unsigned int> &indices, const float *vertices,
                             std::size_t vertexCount, std::size_t stride, float threshold = 1.05f) {
#if BLACKHOLE_ENABLE_MESHOPTIMIZER
  if (indices.empty() || indices.size() % 3 != 0 || vertices == nullptr) {
    return false;
  }
  meshopt_optimizeOverdraw(indices.data(), indices.data(), indices.size(), vertices, vertexCount,
                           stride, threshold);
  return true;
#else
  (void)indices;
...
inline std::size_t optimizeVertexFetch(std::vector<unsigned int> &indices,
  ...
  std::size_t uniqueVertexCount = meshopt_optimizeVertexFetchRemap(remap.data(), indices.data(),
                                                                   indices.size(), vertices.size());

  meshopt_remapIndexBuffer(indices.data(), indices.data(), indices.size(), remap.data());
  meshopt_remapVertexBuffer(vertices.data(), vertices.data(), vertices.size(), sizeof(Vertex),
                            remap.data());
 ...
 */
template <typename Vertex>
inline bool optimizeMesh(std::vector<unsigned int> &indices, std::vector<Vertex> &vertices,
  ...
  meshopt_optimizeOverdraw(indices.data(), indices.data(), indices.size(), positions,
