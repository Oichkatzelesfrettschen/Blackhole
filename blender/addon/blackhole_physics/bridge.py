...
def _find_library():
    ...
    if repo_root is not None:
        candidates.extend([
            ...
            repo_root / "build" / "Release" / "src" / "blender_bridge" / "libblackhole_bridge.so.1.0.0",
            ...
            repo_root / "build" / "BlenderBridge" / "Release" / "src" / "blender_bridge" / "libblackhole_bridge.so.1.0.0",
            repo_root / "build" / "FullDev" / "RelWithDebInfo" / "src" / "blender_bridge" / "libblackhole_bridge.so",
            repo_root / "build" / "FullDev" / "RelWithDebInfo" / "src" / "blender_bridge" / "libblackhole_bridge.so.1.0.0",
        ])
...
def _setup_prototypes(lib):
    ...
    if hasattr(lib, "bhb_cuda_reset_device"):
        lib.bhb_cuda_reset_device.restype = ctypes.c_int
        lib.bhb_cuda_reset_device.argtypes = []
    ...
    if hasattr(lib, "bhb_cuda_render_raytraced_view"):
        lib.bhb_cuda_render_raytraced_view.restype = ctypes.c_int
...
def try_load_library():
    ...
    try:
        ...
        print(f"[Blackhole] Bridge initialization failed for {path}: {e}")
        _lib = None
        _lib_path = None
        return False
...
def cuda_reset_device():
    lib = get_lib()
    fn = getattr(lib, "bhb_cuda_reset_device", None)
    if fn is None:
        return 0
    return int(fn())
