...
(** val local_error_bound :
    RbaseSymbolsImpl.coq_R -> RbaseSymbolsImpl.coq_R -> RbaseSymbolsImpl.coq_R **)

let local_error_bound c h =
  RbaseSymbolsImpl.coq_Rmult c (pow0 h (succ (succ (succ (succ (succ 0))))))
...
(** val check_termination :
    state_vector -> RbaseSymbolsImpl.coq_R -> RbaseSymbolsImpl.coq_R -> int
    -> int -> geodesicStatus **)

let check_termination s r_horizon r_escape step max_steps =
  if rlt_dec (x1 s) r_horizon
  then Captured
  else if rgt_dec (x1 s) r_escape
       then Escaped
       else if Nat.leb max_steps step then MaxSteps else Propagating
...
let rk4_k2 f h y k1 =
  f (sv_add y (sv_scale (rdiv h (iZR (Zpos (XO XH)))) k1))
...
let rk4_k3 f h y k2 =
  f (sv_add y (sv_scale (rdiv h (iZR (Zpos (XO XH)))) k2))
...
(** val rk4_combine :
    RbaseSymbolsImpl.coq_R -> state_vector -> state_vector -> state_vector ->
    state_vector -> state_vector -> state_vector **)

let rk4_combine h y k1 k2 k3 k4 =
  sv_add y
    (sv_scale (rdiv h (iZR (Zpos (XO (XI XH)))))
      (sv_add k1
        (sv_add (sv_scale (iZR (Zpos (XO XH))) k2)
