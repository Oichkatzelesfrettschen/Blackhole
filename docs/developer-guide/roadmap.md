...
| 6.4.5 Exact Kerr Carter constants (GLSL + CUDA) | **COMPLETE (corrected)** | kerrInitGeodesic + d_kerr_init_geodesic solve the null condition for k^t and read E/Lz from the metric. Q = p_theta^2 - a^2 cos^2 + Lz^2 cot^2 is Carter's constant; the earlier Lz^2/sin^2 form made Q exceed Carter's by Lz^2 and shortened R by Delta Lz^2. Gated by kerr_null_geodesic_validation (R(r0) = vr^2, Theta(theta0) = vtheta^2) and kerr_shader_capture (Bardeen capture edges on the GPU) |
| 6.4.6 Second-order Mino stepping and arriving-photon tracing | **COMPLETE** | kerrStep/d_kerr_step integrate d^2r/dlambda^2 = R'/2 and d^2mu/dlambda^2 = Theta_mu'/2 (mu = cos theta, polynomial polar potential) with a kick-drift-kick leapfrog plus a null-constraint projection, so rays pass radial turning points and the poles; the first-order sqrt(max(R,0)) step stalled at turning points. Rays trace in spin -a (kerrTraceSpin): time reversal maps Kerr(a) to Kerr(-a), so the image shows spin +a. One integrator serves every spin including a = 0. The tracer runs in a physics frame (spin along +z) rotated from the y-up world frame of the camera and sky (bhWorldToPhysics) |
