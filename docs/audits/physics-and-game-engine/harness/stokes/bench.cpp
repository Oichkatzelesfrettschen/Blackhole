...
int main() {
  std::vector<Case> cs; std::ifstream in("ref.csv"); std::string line;
  ...
  for (double tf : {0.01, 0.1, 0.5, 1.0, 2.0, 5.0, 10.0, 100.0, 1000.0}) {
    double ee2 = 0, ee = 0, e1 = 0, en = 0, nsum = 0; int cnt = 0;
    for (const auto& c : cs) if (c.tauF == tf) {
  ...
  std::puts("RK4 substeps needed for max_rel < 1e-6 (doubling search), per tauF: mean n, implied ns vs exact");
  for (double tf : {0.01, 1.0, 10.0, 100.0, 1000.0}) {
    double nsum = 0; int cnt = 0;
    for (const auto& c : cs) if (c.tauF == tf) {
      int n = 1; while (err(rk4Sub(c.s0, c.em, c.p, n), c.ref) >= 1e-6 && n < (1 << 22)) n *= 2;
      nsum += n; ++cnt;
    }
    std::printf("  tauF=%-6g mean n=%.0f  -> ~%.0f ns (RK4 18.4 ns/step)\n", tf, nsum / cnt, 18.4 * nsum / cnt);
  }
