...
static S2 deriv(const S2 &s, double mass, double a, const KerrGeodesicConsts &c) {
  ...
  return {s.pr, s.pth, 0.5 * p.dRdr, 0.5 * p.dThetadtheta};
}
int main() {
  ...
  for (double fac : {1.001, 1.0001, 0.999}) {
    ...
    S2 y{50.0, 0.5 * PI, -std::sqrt(p0.rPot), 0.0};
    int n2 = 0; double rmin2 = 1e9, maxRes = 0.0;
    auto t2 = std::chrono::steady_clock::now();
    for (; n2 < 4000000; ++n2) {
      S2 k1 = deriv(y, mass, a, c);
      S2 y2{y.r + 0.5 * dl * k1.r, y.th + 0.5 * dl * k1.th, y.pr + 0.5 * dl * k1.pr, y.pth + 0.5 * dl * k1.pth};
      S2 k2 = deriv(y2, mass, a, c);
      S2 y3{y.r + 0.5 * dl * k2.r, y.th + 0.5 * dl * k2.th, y.pr + 0.5 * dl * k2.pr, y.pth + 0.5 * dl * k2.pth};
      S2 k3 = deriv(y3, mass, a, c);
      S2 y4{y.r + dl * k3.r, y.th + dl * k3.th, y.pr + dl * k3.pr, y.pth + dl * k3.pth};
